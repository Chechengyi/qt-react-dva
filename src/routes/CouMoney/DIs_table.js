...
@Form.create()
@connect(state => ({
  data: state.adminCouMoneyDis.data,
  loading: state.adminCouMoneyDis.loading,
  total: state.adminCouMoneyDis.total,
  ...
  render () {
    ...
    const columns = [
      ...
      },
      {
        title: '申请金额',
        dataIndex: 'putCash',
        render: val=>(<div><span style={{color: '#ff6700'}} >{val}</span> 元</div>)
      },
      {
        title: '转账方式',
        dataIndex: 'accountSign',
        render: val=>(
          <div>
            {val=='alipay'?
